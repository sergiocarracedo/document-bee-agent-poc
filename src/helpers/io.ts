...
export function createConsoleReader({
  ...
  return {
    ...
    async prompt(): Promise<string|void> {
    ...
    async *[Symbol.asyncIterator]() {
      ...
      try {
        ...
        for (let iteration = 1, prompt = ""; isActive; iteration++) {
          prompt = await rl.question(R.piped<string, string, string>(picocolors.cyan, picocolors.bold)(input));
        ...
        if ((e as {code: string}).code === "ERR_USE_AFTER_CLOSE") {
          return;
        }
        throw e;
      } finally {
